...
{
    LL res=0;
    pre[0]=(v-x[0]+mod)%mod,suf[n]=1;
    for(int i=1;i<n;i++) pre[i]=pre[i-1]*(v-x[i])%mod;
    for(int i=n-1;i>=0;i--) suf[i]=suf[i+1]*(v-x[i])%mod;
    inv[0]=inv[1]=1;
    for(int i=2;i<n;i++) inv[i]=(mod-mod/i)*inv[mod%i]%mod;
    for(int i=2;i<n;i++) inv[i]=inv[i]*inv[i-1]%mod;
    for(int i=0;i<n;i++) res+=y[i]*(i==0?1:pre[i-1])%mod*suf[i+1]%mod*inv[i]%mod*(((n-i)&1)?mod-1:1)*inv[n-i]%mod,res%=mod;
    return res;
}*/
